...
def build_system_prompt(assistant_name, personality):
    ...
    return (
        ...
        '记账时的「分类」字段只能从这几个里选一个：餐饮、交通、购物、缴费、娱乐、医疗、学习工作、其他。'
        '请始终返回 JSON 格式：'
        '{"reply":"给用户的话（口语化）","should_save":true/false,'
        '"record":{"amount":数字,"entry_type":"expense|income","category":"餐饮|交通|购物|缴费|娱乐|医疗|学习工作|其他","note":"备注","occurred_at":"YYYY-MM-DD"}}。'
...
def fallback_parse_accounting(user_text):
    ...
    category = '其他'
    category_rules = {
        '餐饮': ['饭', '餐', '吃', '外卖', '奶茶', '咖啡', '早餐', '午餐', '晚餐', '宵夜'],
        '交通': ['地铁', '公交', '打车', '出租', '滴滴', '网约车', '加油', '油费', '停车', '高铁', '火车', '飞机'],
        '购物': ['买', '购物', '超市', '淘宝', '京东', '拼多多', '网购', '衣服', '鞋', '日用品'],
        '缴费': ['水费', '电费', '话费', '网费', '宽带', '燃气', '煤气', '物业', '取暖', '供暖'],
        '娱乐': ['电影', '游戏', 'KTV', '旅游', '演唱会', '娱乐', '聚会'],
        '医疗': ['医院', '看病', '挂号', '药', '体检', '牙科', '治疗'],
        '学习工作': ['学习', '课程', '网课', '培训', '教材', '书', '办公', '软件', '工作', '工具']
    }
